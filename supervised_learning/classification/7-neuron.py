...
class Neuron:
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05):
        ...
        for i in range(iterations + 1):
            # Forward prop to update self.__A
            self.forward_prop(X)
            curr_cost = self.cost(Y, self.__A)

            # Record/Print data at specified steps and final iteration
            ...
            # Perform gradient descent (except on the last iteration)
            if i < iterations:
