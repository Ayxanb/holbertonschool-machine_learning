...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        """
        for i in range(len(boxes)):
            # Coordinates for drawing (int pixels)
            x1, y1, x2, y2 = boxes[i].astype(int)

            # Draw the boundary box: Blue (BGR: 255, 0, 0), thickness 2
            cv2.rectangle(image, (x1, y1), (x2, y2), (255, 0, 0), 2)

            # Format text: Class Name and Score rounded to 2 decimals
            class_name = self.class_names[box_classes[i]]
            score = box_scores[i]
            label = "{} {:.2f}".format(class_name, score)

            # Draw text: Red (BGR: 0, 0, 255), 5px above top-left corner
            # FONT_HERSHEY_SIMPLEX, scale 0.5, thickness 1, LINE_AA
            cv2.putText(image, label, (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX,
                        0.5, (0, 0, 255), 1, cv2.LINE_AA)

        # Display image in a window named after the file_name
        ...
        if key == ord('s'):
            ...
            # Save using the original file_name inside 'detections/'
