...
class Decision_Tree:
    ...
    def possible_thresholds(self, node, feature):
        """
        Calculates all midpoints between unique feature values as thresholds.
        """
    ...
    def Gini_split_criterion_one_feature(self, node, feature):
        """
        Calculates the best Gini split for a single feature without loops.
        """
        # Data for the current node
        pop_indices = node.sub_population
        feat_values = self.explanatory[pop_indices, feature]
        targets = self.target[pop_indices]
        classes = np.unique(targets)
        thresholds = self.possible_thresholds(node, feature)

        # n: individuals, t: thresholds, c: classes
        # Broadcasting targets and feat_values to shape (n, t, c)
        # 1. Is individual i of class k? (n, 1, c)
        class_mask = targets[:, np.newaxis] == classes
        class_mask = class_mask[:, np.newaxis, :]

        # 2. Is feature value of individual i > threshold j? (n, t, 1)
        thresh_mask = feat_values[:, np.newaxis] > thresholds
        thresh_mask = thresh_mask[:, :, np.newaxis]

        # Left_F[i, j, k]: individual i is class k AND feature > threshold j
        left_f = np.logical_and(class_mask, thresh_mask)
        # Right_F: individual i is class k AND feature <= threshold j
        right_f = np.logical_and(class_mask, ~thresh_mask)

        # Count occurrences (sum along individuals axis 'n')
        count_left = np.sum(left_f, axis=0)   # Shape (t, c)
        count_right = np.sum(right_f, axis=0)  # Shape (t, c)

        # Total population in each side for each threshold
        pop_left = np.sum(count_left, axis=1)   # Shape (t,)
        pop_right = np.sum(count_right, axis=1)  # Shape (t,)
        total_pop = len(targets)

        # Gini = 1 - sum((count / pop)^2)
        # We use np.divide and handle division by zero with where
        gini_l = 1 - np.sum(np.square(np.divide(
            count_left, pop_left[:, np.newaxis],
            out=np.zeros_like(count_left, dtype=float),
            where=pop_left[:, np.newaxis] != 0)), axis=1)

        gini_r = 1 - np.sum(np.square(np.divide(
            count_right, pop_right[:, np.newaxis],
            out=np.zeros_like(count_right, dtype=float),
            where=pop_right[:, np.newaxis] != 0)), axis=1)

        # Weighted average of Gini impurities
        gini_total = (pop_left / total_pop) * gini_l + \
                     (pop_right / total_pop) * gini_r
    ...
    def Gini_split_criterion(self, node):
        """
        Determines the best feature and threshold using Gini impurity.
        """
        # Map results for each feature
        x = np.array([self.Gini_split_criterion_one_feature(node, i)
                      for i in range(self.explanatory.shape[1])])
        # Find feature with the lowest Gini index (index 1 in the tuples)
        i = np.argmin(x[:, 1])
        return i, x[i, 0]
